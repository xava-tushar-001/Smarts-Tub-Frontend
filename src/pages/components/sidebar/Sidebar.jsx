...
export default function Sidebar() {
  ...
  return (
    <aside
      className={`flex h-full min-h-0 shrink-0 flex-col overflow-hidden bg-white shadow-lg shadow-slate-200/50 transition-[width] duration-300 ease-in-out ${
        open ? "w-64" : "w-[4.75rem]"
      }`}
    >
      {/* Header Section with Logo */}
      <div
        className={`flex min-h-[4.5rem] items-center border-b border-slate-100 px-3 py-4 ${
        ...
        {sidebarMenu.map((item) => {
          ...
          return (
            <NavLink
              ...
              className={({ isActive }) =>
                `${linkClass} ${!open ? "justify-center px-2" : ""} ${
      ...
      <div className="shrink-0 border-t border-slate-100 p-3">
        <button
          ...
          className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-500 transition-all duration-200 hover:bg-blue-50 hover:text-blue-600 ${
